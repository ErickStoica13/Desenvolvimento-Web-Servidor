...
if (!isset($_SESSION['logado']) || $_SESSION['logado'] !== true) {
    header('Location: index.php?acao=erro-compra');
    exit; 
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['finalizar-compra'])) {
    ...
    if (
        ...
        if (strlen($_POST['numero_cartao']) !== 16 || !ctype_digit($_POST['numero_cartao'])) {
            header('Location: index.php?acao=erro-campo-cartao');
            exit;
        }

        //validando o cvv 3 digitos apenas
        if (strlen($_POST['cvv']) !== 3 || !ctype_digit($_POST['cvv'])) {
            header('Location: index.php?acao=erro-campo-cvv');
            exit;
        }
        ...
        if ($dataValidade <= $dataAtual) {
            header('Location: index.php?acao=erro-campo-data');
            exit;
        }
        ...
        if (!empty($_SESSION['carrinho'])) {
            ...
            header('Location: index.php?acao=compra-finalizada');
        } else {
            header('Location: index.php?acao=erro-item');
            exit;
        }
    } else {
